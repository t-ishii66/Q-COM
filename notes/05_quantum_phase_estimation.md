...
具体的には、$\varphi$ の $n$ ビットの近似 $\tilde{\varphi}$ を求める。$n$ ビットの精度とは $\tilde{\varphi} = 0.j_1 j_2 \cdots j_n$（2進小数）の意味であり、誤差は $\lvert\varphi - \tilde{\varphi}\rvert \le 2^{-n}$ 程度となる。
...
$$
\frac{\vert 0\rangle + \vert 1\rangle}{\sqrt{2}}\vert u\rangle \xrightarrow{CU} \frac{\vert 0\rangle + e^{2\pi i \varphi}\vert 1\rangle}{\sqrt{2}}\vert u\rangle
$$

$n = 2$ の場合、$q_1$ で $U^2$ を、$q_2$ で $U^1$ を制御する：

$$
\frac{\vert 0\rangle + \vert 1\rangle}{\sqrt{2}} \otimes \frac{\vert 0\rangle + \vert 1\rangle}{\sqrt{2}} \otimes \vert u\rangle \xrightarrow{CU^2, CU} \frac{\vert 0\rangle + e^{2\pi i \cdot 2\varphi}\vert 1\rangle}{\sqrt{2}} \otimes \frac{\vert 0\rangle + e^{2\pi i \varphi}\vert 1\rangle}{\sqrt{2}} \otimes \vert u\rangle
$$
...
$$
\varphi = \frac{s}{2^n} = 0.j_1 j_2 \cdots j_n \quad \text{（$s$ の2進数表現）}
$$
...
$$
\vert 0\rangle^{\otimes n}\vert u\rangle \xrightarrow{H^{\otimes n}} \frac{1}{\sqrt{N}}\sum_j \vert j\rangle\vert u\rangle \xrightarrow{CU^{2^k}} \frac{1}{\sqrt{N}}\sum_j e^{2\pi ij\varphi}\vert j\rangle\vert u\rangle \xrightarrow{\text{QFT}^{-1}} \vert\tilde{s}\rangle\vert u\rangle
$$
...
| 精度 | $n$ ビットの計数レジスタで $\lvert\varphi - \tilde{\varphi}\rvert \le 2^{-n}$ |
